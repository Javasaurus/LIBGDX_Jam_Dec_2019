...
import be.codecuisine.game.enemies.ships.impl.dev.SimpleChaseEnemy;
import be.codecuisine.game.enemies.ships.impl.dev.SimpleChaseHeadEnemy;
import be.codecuisine.game.enemies.ships.impl.dev.SimpleWaypointEnemy;
 ...
 * @author kenne
 */
public class BasicSnakeShip extends SimpleWaypointEnemy {

    public BaseEnemy chain[];
    private final int chainLength;

    public BasicSnakeShip(Vector2 position, GameBehavior target, int chainLength, float sizeX, float sizeY, int shape) {
        super(position, sizeX, sizeY, shape);
        this.chain = new BaseEnemy[chainLength];
        GameBehavior tmp = this;
        chain[0] = new SimpleChaseHeadEnemy(new Vector2(position), tmp, sizeX, sizeY);
